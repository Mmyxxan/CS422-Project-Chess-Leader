...
class NNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.residual_blocks = nn.ModuleList([ResidualBlock(self.NUM_FILTERS) for _ in range(self.NUM_RESIDUAL_BLOCKS)])
        self.policy_head = PolicyHead(self.NUM_FILTERS)
        self.value_head = ValueHead(self.NUM_FILTERS)
        # self.softmax = nn.Softmax(1)
        self.log_softmax = nn.LogSoftmax(1)
        self.mse_loss = nn.MSELoss() # user-defined for certainty?
    ...
    def forward(self, x):
        ...
        # return policy, value
        return self.log_softmax(policy), value
